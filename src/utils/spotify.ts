...
import { downloadYoutubeSog, searchYoutubeSong } from "./youtube";
...
export const searchSpotifySong = async (url: string): Promise<MySong> => {
  ...
  return {
    url: url,
    title: track.name,
    authors: track.artists.toString(),
    name: track.name,
...
export const searchSpotifyPlaylist = async (
  ...
  const { name, total_tracks, tracks } = await spotify.getTracksFromPlaylist(
    url
  );
  return {
    url: url,
...
export const downloadSpotifySong = async (song: MySong) => {
  ...
  const newSong = await searchYoutubeSong(song.name + " by " + song.authors);
  return await downloadYoutubeSog(newSong);
};
